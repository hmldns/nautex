...
Binary: kiro-cli acp
Transport: stdio
Auth: UNKNOWN — may use AWS Builder ID
Note: Directory scope via --directory flag per MDS spec
"""
...
async def run(prompt: str, model: str | None, timeout: int, workspace: str | None = None, keep: bool = False, consolidate: bool = False):
    ...
    print(f"{C.BOLD}=== Probe: Kiro CLI ==={C.RESET}")
...
def main():
    parser = argparse.ArgumentParser(description="Kiro CLI ACP probe")
