...
Binary: goose acp
Transport: stdio
Auth: UNKNOWN — may use configured provider creds
Note: Model override via GOOSE_MODEL env var per MDS spec
"""
...
async def run(prompt: str, model: str | None, timeout: int, workspace: str | None = None, keep: bool = False):
    ...
    print(f"{C.BOLD}=== Probe: Goose ==={C.RESET}")
    ...
    try:
        # Goose needs full host env — SDK's default_environment strips API keys
        async with spawn_agent_process(client, CMD, *ARGS, cwd=tmpdir, env=dict(os.environ)) as (conn, proc):
...
def main():
    parser = argparse.ArgumentParser(description="Goose ACP probe")
