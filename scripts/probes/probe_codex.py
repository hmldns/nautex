...
Binary: codex-acp (@zed-industries/codex-acp)
Transport: stdio
Auth: UNKNOWN — may use OPENAI_API_KEY or cached creds
"""
...
async def run(prompt: str, model: str | None, timeout: int, workspace: str | None = None, keep: bool = False):
    ...
    print(f"{C.BOLD}=== Probe: Codex ==={C.RESET}")
...
def main():
    parser = argparse.ArgumentParser(description="Codex ACP probe")
