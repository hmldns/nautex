...
-   **{TaskType.ALIGN.value}**: This task requires the agent to reconcile spec/plan intent with the existing codebase before implementation. The `description` will instruct the agent on what relevant patterns, modules, data models, and integration points to inspect. The agent must report gaps, contradictions, and integration concerns to the user, and use the `{CMD_SUBMIT_CHANGE_REQUEST}` command to propose solutions so the spec stays consistent with codebase reality. Do not mark as 'Done' until the user explicitly confirms the alignment findings and any decisions. Typically appears as the first task under each Phase when improving existing codebases.
