#!/usr/bin/env python3
"""Droid (Factory) ACP probe.

Binary: droid exec --output-format acp
Transport: stdio (exec mode, not daemon)
Auth: UNKNOWN
Note: MDS spec says HTTP daemon with --port 0, but exec mode may use stdio.
"""
    ...
    try:
        async with spawn_agent_process(client, CMD, *ARGS, cwd=tmpdir) as (conn, proc):
            ...
            try:
                ...
                log("init", C.RED, "TIMEOUT — droid may not support ACP over stdio with these args")
                return
...
def main():
    parser = argparse.ArgumentParser(description="Droid ACP probe")
