"""Grok Build agent setup and configuration (file-based MCP + AGENTS.md)."""
...
# Grok-specific MCP server fields (project-local .grok/config.toml).
# startup_timeout_sec=20: uvx is usually fast; avoid long hangs on failure.
GROK_MCP_ENTRY_EXTRAS = {
    "enabled": True,
    "startup_timeout_sec": 20,
}


class GrokAgentSetup(ConfigAwareSectionManagedRulesMixin, FilesBasedMCPAgentSetup):
    """Grok Build agent setup and configuration.

    - Uses the project-local TOML MCP config at `.grok/config.toml`.
      Before first write, creates a backup `config.toml.bak` alongside if absent.
    - Manages `.nautex/AGENTS.md` with full rules and a root `AGENTS.md`
      including a managed reference section (Grok auto-loads AGENTS.md).
    """

    def __init__(self, config_service):
        super().__init__(config_service, AgentType.GROK)
        self.config_folder = Path(".grok")
    ...
    def get_agent_mcp_config_path(self) -> Path:
        """Path to the project-local MCP configuration file for Grok Build (TOML)."""
        return self.cwd / ".grok" / "config.toml"

    def get_agent_mcp_backup_path(self) -> Path:
        """Backup path for the Grok MCP configuration file."""
    ...
    async def get_mcp_configuration_info(self) -> str:
        ...
        return (
            f"Grok Build MCP config (project): {path2display(cfg)}\n"
    ...
    async def write_mcp_configuration(self) -> bool:
        ...
        return await asyncio.to_thread(
            ...
            full_path,
            self.cwd,
            entry_extras=GROK_MCP_ENTRY_EXTRAS,
        )

    # ---------- Rules paths ----------
    def get_rules_path(self) -> Path:
        """Full rules content lives in `.nautex/AGENTS.md`."""
        return self.cwd / Path(DIR_NAUTEX) / self.rules_filename

    @property
    def root_agent_path(self) -> Path:
        """Path to the root AGENTS.md file (with reference section)."""
    ...
    def get_default_rules_template(self) -> str:
        return default_agents_rules_template_for("AGENTS.md", AgentType.GROK.display_name())
